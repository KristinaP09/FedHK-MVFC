%%   Federated Data Partitioning Script for FedHK-MVFC
...
%%   ------------------------------------------------------------------- %%
%%   Modified: Aug 13, 2025 - Federated partitioning for synthetic dataset
%%   
%%   Based on LaTeX document guidelines (line 985):
%%   - Client 1: 8,500 samples (85% of data)
%%   - Client 2: 1,500 samples (15% of data)  
%%   - Stratified sampling ensures both clients access all four cluster types
%%   - Reflects realistic federated scenarios with heterogeneous data distributions

clear all; close all; clc;

%% Load the synthetic dataset
fprintf('=== FEDERATED DATA PARTITIONING FOR FedHK-MVFC ===\n');
fprintf('Loading synthetic dataset...\n');

% Check if the expanded dataset exists, if not use the compact version
if exist('data_distinct_shapes_expanded.mat', 'file')
    load('data_distinct_shapes_expanded.mat');
    fprintf(' Loaded expanded dataset: data_distinct_shapes_expanded.mat\n');
else
    load('data_distinct_shapes.mat');
    fprintf(' Loaded compact dataset: data_distinct_shapes.mat\n');
end
...
% Based on LaTeX guidelines: Client 1 gets 8,500 samples, Client 2 gets 1,500 samples
% Ensure both clients have access to all four cluster types

client1_size = 8500;  % 85% of data
client2_size = 1500;  % 15% of data
...
save('client1_data.mat', 'X_client1', 'label_client1', 'client1_indices');
save('client2_data.mat', 'X_client2', 'label_client2', 'client2_indices');

% Save combined federated structure
save('federated_data.mat', 'X_clients', 'P', 'points', 'label', ...
     'client1_indices', 'client2_indices', 'X_client1', 'X_client2', ...
     'label_client1', 'label_client2');

fprintf(' Saved federated datasets:\n');
fprintf('  - client1_data.mat (Client 1 data)\n');
fprintf('  - client2_data.mat (Client 2 data)\n');
fprintf('  - federated_data.mat (Combined federated structure)\n');
